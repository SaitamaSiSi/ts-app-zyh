...
export default {
  ...
  methods: {
    init() {
      // 获取所有的省份
      /*
      const res = await this.$api.province();
      this.provinces = [];
      if (res.code == 0) {
      ...
      this.provinces = [];
      for (let i = 0; i < 15; i++) {
        this.provinces.push({ label: "省份" + i, value: "" + i });
      }
    ...
    provinceChange(index) {
      ...
      // 接口读取
      /*
      const res = await this.$api.nextRegion(this.provinces[index].value);
      this.citys = [];
      if (res.code == 0) {
        res.data.forEach((ele) => {
          this.citys.push({
    ...
    cityChange(index) {
      ...
      // 接口读取
      /*
      const res = await this.$api.nextRegion(this.citys[index].value);
      this.areas = [];
      if (res.code == 0) {
        res.data.forEach((ele) => {
          this.areas.push({
